           ...
           int        *useraction_p)
{
   ...
   *useraction_p = CPX_CALLBACK_DEFAULT;
   ...
   if ( nodecount < 1 ) {
      ...
      status = CPXgetintparam (env, CPXPARAM_Simplex_Limits_Iterations, &itlimit);
      if ( status )  goto TERMINATE;

      status = CPXsetintparam ((CPXENVptr) env, CPXPARAM_Simplex_Limits_Iterations, 200);
      if ( status )  goto TERMINATE;

      status = CPXdualopt (env, nodelp);

      CPXsetintparam ((CPXENVptr) env, CPXPARAM_Simplex_Limits_Iterations, itlimit);
      ...
      if ( !status ) {
         ...
         if ( solstat == CPX_STAT_ABORT_IT_LIM ) {
            ...
            status = CPXgetintparam (env, CPXPARAM_Preprocessing_Presolve, &preind);
            if ( status )  goto TERMINATE;
            status = CPXgetintparam (env, CPXPARAM_Preprocessing_Aggregator, &aggind);
            if ( status )  goto TERMINATE;

            status = CPXsetintparam ((CPXENVptr) env, CPXPARAM_Preprocessing_Presolve, CPX_ON);
            if ( status )  goto TERMINATE;
            status = CPXsetintparam ((CPXENVptr) env, CPXPARAM_Preprocessing_Aggregator, -1);
            if ( status )  goto TERMINATE;

            status = CPXhybbaropt (env, nodelp, CPX_ALG_AUTOMATIC);

            CPXsetintparam ((CPXENVptr) env, CPXPARAM_Preprocessing_Presolve, preind);
            CPXsetintparam ((CPXENVptr) env, CPXPARAM_Preprocessing_Aggregator, aggind);
         }
      }
   }
