 ...
 *                                                                        *
 *    On the wokerr we register a user function that installs an info     *
 ...
 *    the callable library's C API, not with the C++ API.                 *
 *                                                                        *
 * ********************************************************************** */
...
class Worker {
   ...
   struct InfoHandler : public IloCplex::RemoteInfoHandler {
                ...
                void const *data)
      {
         ...
         case INFO_NEWDUAL:
            ...
            std::cerr << "[" << worker->idx << "] New dual bound: " << d
                      << std::endl;
            worker->dual = d;
         ...
         case INFO_NEWPRIMAL:
            ...
            assert (length == 1);
            d = *static_cast<double const *>(data);
            std::cerr << "[" << worker->idx << "] New primal bound: " << d
                      << std::endl;
            worker->primal = d;
         ...
         case INFO_DETTIME:
            ...
            assert (length == 1);
            d = *static_cast<double const *>(data);
            std::cerr << "[" << worker->idx << "] Current dettime: " << d
                      << " (" << worker->dual << ", " << worker->primal << ")"
                      << std::endl;
            worker->dettime = d;
            break;
         default:
            std::cerr << "[" << worker->idx << "] Unknown info " << tag
                      << std::endl;
         }
      }
   };
   ...
   bool mutable finished;         /**< True if the worker has finished. */
...
public:
   ...
   Worker(IloEnv env, int i, SolveState *s, char const *transport,
      ...
      : idx(i), state(s), model(env), cplex(0), handle(0),
        primal(IloInfinity), dual(-IloInfinity), finished(false),
   ...
   /** Test if this worker has finished (and a call to join() will not block).
    */
   ...
   void kill() { handle.kill(); finished = true; }
   /** Join this workers.
