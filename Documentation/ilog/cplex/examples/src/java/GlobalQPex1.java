 ...
 * That is:   java GlobalQPex1  filename solutiontarget
 * where 
...
public class GlobalQPex1 {
   static void usage() {
      System.out.println("usage:  GlobalQPex1 <filename> <solution target>");
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         // Evaluate command line option and set solution target accordingly.
         switch ( args[1].charAt(0) ) {
         case 'c': cplex.setParam(IloCplex.Param.SolutionTarget,
                                  IloCplex.SolutionTarget.OptimalConvex);
                   break;
         case 'f': cplex.setParam(IloCplex.Param.SolutionTarget,
                                  IloCplex.SolutionTarget.FirstOrder);
                   break;
         case 'g': cplex.setParam(IloCplex.Param.SolutionTarget,
                                  IloCplex.SolutionTarget.OptimalGlobal);
                   break;
         default:  usage();
                   return;
         }
