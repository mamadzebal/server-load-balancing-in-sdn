           ...
           int        *useraction_p)
{
   ...
   *useraction_p = CPX_CALLBACK_DEFAULT;
   ...
   if ( nodecount < 1 ) {
      ...
      status = CPXXgetcntparam (env, CPXPARAM_Simplex_Limits_Iterations, &itlimit);
      if ( status )  goto TERMINATE;

      status = CPXXsetcntparam ((CPXENVptr) env, CPXPARAM_Simplex_Limits_Iterations, 200);
      if ( status )  goto TERMINATE;

      status = CPXXdualopt (env, nodelp);

      CPXXsetcntparam ((CPXENVptr) env, CPXPARAM_Simplex_Limits_Iterations, itlimit);
      ...
      if ( !status ) {
         ...
         if ( solstat == CPX_STAT_ABORT_IT_LIM ) {
            ...
            status = CPXXgetintparam (env, CPXPARAM_Preprocessing_Presolve, &preind);
            if ( status )  goto TERMINATE;
            status = CPXXgetintparam (env, CPXPARAM_Preprocessing_Aggregator, &aggind);
            if ( status )  goto TERMINATE;

            status = CPXXsetintparam ((CPXENVptr) env, CPXPARAM_Preprocessing_Presolve, CPX_ON);
            if ( status )  goto TERMINATE;
            status = CPXXsetintparam ((CPXENVptr) env, CPXPARAM_Preprocessing_Aggregator, -1);
            if ( status )  goto TERMINATE;

            status = CPXXhybbaropt (env, nodelp, CPX_ALG_AUTOMATIC);

            CPXXsetintparam ((CPXENVptr) env, CPXPARAM_Preprocessing_Presolve, preind);
            CPXXsetintparam ((CPXENVptr) env, CPXPARAM_Preprocessing_Aggregator, aggind);
         }
      }
   }
