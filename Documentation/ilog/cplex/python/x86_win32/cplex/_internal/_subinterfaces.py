...
class Histogram(object):
    def __init__(self, c, key):
        ...
        if self.__hist is None:
            raise CplexError("Error generating histogram")
...
class BaseInterface(object):
    ...
    def _conv(self, name):
        """non-public"""
        if isinstance(name, type("")) or isinstance(name, type(u"")):
...
class IndexedInterface(BaseInterface):
    ...
    def get_indices(self, name):
        ...
        if isinstance(name, type("")):
            return self._get_index_function(self._env._e, self._cplex._lp, name, self._env.parameters.read.apiencoding.get())
        elif isinstance(name, type(u"")):
            return self._get_index_function(self._env._e, self._cplex._lp, CPX_PROC.cpx_decode(name, CPX_PROC.cpx_default_encoding), CPX_PROC.cpx_default_encoding)
        else:
            return map(lambda a: self._get_index_function(self._env._e, self._cplex._lp, a, self._env.parameters.read.apiencoding.get()), name)
...
class AdvancedVariablesInterface(AdvancedInterface):
    ...
    def protect(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = ["a", "b", "c", "d"])
        >>> c.variables.advanced.protect("a")
        >>> c.variables.advanced.protect(["b", "d"])

        """
        if isinstance(self._parent._conv(args[0]), type(0)):
            a = [self._parent._conv(args[0])]
        else:
            a = map(self._parent._conv, args[0])
    ...
    def tighten_lower_bounds(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = ["x0", "x1", "x2"])
...
class VarTypes:
    """Constants defining variable types
       ...
       For a definition of each type, see those topics in the 
       CPLEX User's Manual.
...
class VariablesInterface(IndexedInterface):
    ...
    def get_num(self):
        ...
        >>> c = cplex.Cplex()
        >>> t = c.variables.type
        >>> c.variables.add(types = [t.continuous, t.binary, t.integer])
        >>> c.variables.get_num()
        3
    ...
    def get_num_binary(self):
        ...
        >>> c = cplex.Cplex()
        >>> t = c.variables.type
        >>> c.variables.add(types = [t.semi_continuous, t.binary, t.integer])
        >>> c.variables.get_num_binary()
        1
    ...
    def get_num_semicontinuous(self):
        ...
        >>> c = cplex.Cplex()
        >>> t = c.variables.type
        >>> c.variables.add(types = [t.semi_continuous, t.semi_integer, t.semi_integer])
    ...
    def add(self, obj = [], lb = [], ub = [], types = "", names = [], columns = []):
        ...
        if not isinstance(types, type("")):
            types = "".join(types)
        num_new_cols = validate_arg_lengths(self._env, "variables.add", [obj, lb, ub, types, names, columns])
        num_old_cols = self.get_num()
        if columns == []:
            CPX_PROC.newcols(self._env._e, self._cplex._lp, obj, lb, ub, types, names, self._env.parameters.read.apiencoding.get())
        else:
            cmat = _C_HBMatrix(columns, self._cplex._env_lp_ptr, 1, self._env.parameters.read.apiencoding.get())
            CPX_PROC.addcols(self._env._e, self._cplex._lp, num_new_cols, cmat._get_nnz(), obj,
                             cmat, lb, ub, names, self._env.parameters.read.apiencoding.get())
            if types != "":
                CPX_PROC.chgctype(self._env._e, self._cplex._lp, range(num_old_cols, num_old_cols + num_new_cols), types)
    ...
    def delete(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(10)])
        >>> c.variables.get_num()
        10
    ...
    def set_types(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(5)])
        ...
        """
        if len(args) == 2:
            CPX_PROC.chgctype(self._env._e, self._cplex._lp, [self._conv(args[0])], args[1])
        else:
            a1, a2 = zip(*args[0])
            CPX_PROC.chgctype(self._env._e, self._cplex._lp, map(self._conv, a1), "".join(a2))
    ...
    def get_lower_bounds(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(lb = [1.5 * i for i in range(10)],\
    ...
    def get_upper_bounds(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(ub = [(1.5 * i) + 1.0 for i in range(10)],\
                            names = [str(i) for i in range(10)])
        >>> c.variables.get_num()
        10
    ...
    def get_names(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = ['x' + str(i) for i in range(10)])
        >>> c.variables.get_num()
        10
        ...
        def getname(a, b = self.get_num() - 1):
            return CPX_PROC.getcolname(self._env._e, self._cplex._lp, a, b, self._env.parameters.read.apiencoding.get())
        return apply_freeform_two_args("variables.get_names", getname, self.get_indices, args)
    ...
    def get_types(self, *args):
        ...
        t = [i for i in "".join(apply_freeform_two_args("variables.get_type", gettype, self.get_indices, args))]
        if len(t) == 1:
            return t[0]
        else:
            return t
    ...
    def get_cols(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.linear_constraints.add(names = ['c1', 'c2'])
        >>> c.variables.add(names = [str(i) for i in range(3)],\
...
class AdvancedLinearConstraintInterface(AdvancedInterface):
    ...
    """Methods for handling lazy cuts and user cuts.
      ...
      the CPLEX parameter cplex.parameters.preprocessing.linear to 0 (zero)  
      to make sure that CPLEX makes only linear reductions.
    ...
    def add_lazy_cuts(self, lin_expr = [], senses = "", rhs = [], names = []):
        ...
        >>> c.linear_constraints.advanced.add_lazy_cuts(names = ["lz1"],\
                                                        lin_expr = [cut],\
                                                        senses = "E",\
                                                        rhs = [0.0])
        >>> cut2 = cplex.SparsePair(ind = [0,2,4],val = [1.0,1.0,1.0])
        >>> cut3 = cplex.SparsePair(ind = [0,2,5],val = [1.0,1.0,1.0])
        >>> c.linear_constraints.advanced.add_lazy_cuts(names = ["lz2", "lz3"],\
                                                        lin_expr = [cut2, cut3],\
                                                        senses = "EE",\
                                                        rhs = [0.0,0.0])

        """
        if not isinstance(senses, type("")):
            senses = "".join(senses)
        num_lazy_cuts = validate_arg_lengths(self._env, "linear_constraints.advanced.add_lazy_cuts",
                                             [rhs, senses, names, lin_expr])
        if isinstance(lin_expr, type([])):
            rmat = _HBMatrix(lin_expr)
        CPX_PROC.addlazyconstraints(self._env._e, self._cplex._lp, rhs, senses,
                                    rmat.matbeg, map(self._cplex.variables._conv, rmat.matind),
    ...
    def add_user_cuts(self, lin_expr = [], senses = "", rhs = [], names = []):
        ...
        >>> c.variables.add(names = [str(i) for i in range(10)])
        >>> cut = cplex.SparsePair(ind = [0,1,4],val = [1.0,1.0,1.0])
        >>> c.linear_constraints.advanced.add_user_cuts(names = ["usr1"],\
                                                        lin_expr = [cut],\
                                                        senses = "E",\
                                                        rhs = [0.0], )
        >>> cut2 = cplex.SparsePair(ind = [0,2,4],val = [1.0,1.0,1.0])
        >>> cut3 = cplex.SparsePair(ind = [0,2,5],val = [1.0,1.0,1.0])
        >>> c.linear_constraints.advanced.add_lazy_cuts(names = ["usr2", "usr3"],\
                                                        lin_expr = [cut2, cut3],\
                                                        senses = "EE",\
                                                        rhs = [0.0,0.0])
        ...
        if not isinstance(senses, type("")):
            senses = "".join(senses)
        num_user_cuts = validate_arg_lengths(self._env,  "linear_constraints.advanced.add_user_cuts",
                                             [rhs, senses, names, lin_expr])
        if isinstance(lin_expr, type([])):
            rmat = _HBMatrix(lin_expr)
        CPX_PROC.addusercuts(self._env._e, self._cplex._lp, rhs, senses,
                             rmat.matbeg, map(self._cplex.variables._conv, rmat.matind),
                             rmat.matval, names, self._env.parameters.read.apiencoding.get())
...
class LinearConstraintInterface(IndexedInterface):
    ...
    def get_num(self):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.linear_constraints.add(names = ["c1", "c2", "c3"])
    ...
    def add(self, lin_expr = [], senses = "", rhs = [], range_values = [], names = []):
        ...
        if not isinstance(senses, type("")):
            senses = "".join(senses)
        num_new_rows = validate_arg_lengths(self._env, "linear_constraints.add", [rhs, senses, range_values, names, lin_expr])
        num_old_rows = self.get_num()
        if lin_expr == []:
            if senses.find('R') != -1 and len(range_values) == 0:
                range_values = [0.0] * len(senses)
            CPX_PROC.newrows(self._env._e, self._cplex._lp, rhs, senses, range_values, names, self._env.parameters.read.apiencoding.get())
        else:
            rmat = _C_HBMatrix(lin_expr, self._cplex._env_lp_ptr, 0, self._env.parameters.read.apiencoding.get())
            CPX_PROC.addrows(self._env._e, self._cplex._lp, 0, num_new_rows, rmat._get_nnz(), rhs,
                                 senses, rmat, [], names, self._env.parameters.read.apiencoding.get())
            if range_values != []:
                CPX_PROC.chgrngval(self._env._e, self._cplex._lp, range(num_old_rows, num_old_rows + num_new_rows), range_values)
    ...
    def delete(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.linear_constraints.add(names = [str(i) for i in range(10)])
        >>> c.linear_constraints.get_num()
        10
    ...
    def set_rhs(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.linear_constraints.add(names = ["c0", "c1", "c2", "c3"])
        >>> c.linear_constraints.get_rhs()
        [0.0, 0.0, 0.0, 0.0]
    ...
    def set_senses(self, *args):
        ...
        """
        if len(args) == 2:
            CPX_PROC.chgsense(self._env._e, self._cplex._lp, [self._conv(args[0])], args[1])
        else:
            a1, a2 = zip(*args[0])
            CPX_PROC.chgsense(self._env._e, self._cplex._lp, map(self._conv, a1), "".join(a2))
        ...
        def setlin(aa, bb):
            for i, a in enumerate(aa):
                ...
                if isinstance(b, SparsePair):
                    ...
                    val = b[1]
                if len(ind) != len(val):
                    raise CplexError("Inconsistent arguments to linear_constraints.set_linear_components")
                CPX_PROC.chgcoeflist(self._env._e, self._cplex._lp, [self._conv(a)] * len(ind),
                                     map(self._cplex.variables._conv, ind), val)
        apply_pairs("linear_constraints.set_lin", setlin, self.get_indices, *args)
    ...
    def set_coefficients(self, *args):
        ...
        """
        if isinstance(self._conv(args[0]), type(0)):
            arg_list = [[arg] for arg in args]
        else:
            arg_list = zip(*args[0])
        CPX_PROC.chgcoeflist(self._env._e, self._cplex._lp, map(self._conv, arg_list[0]),
                         map(self._cplex.variables._conv, arg_list[1]), arg_list[2])
    ...
    def get_rhs(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.linear_constraints.add(rhs = [1.5 * i for i in range(10)],\
                                     names = [str(i) for i in range(10)])
        >>> c.linear_constraints.get_num()
        10
    ...
    def get_senses(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.linear_constraints.add(senses = ["E", "G", "L", "R"],\
        ...
        s = [i for i in "".join(apply_freeform_two_args("linear_constraints.get_senses", getsense, self.get_indices, args))]
        if len(s) == 1:
            return s[0]
        else:
            return s
    ...
    def get_range_values(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.linear_constraints.add(range_values = [1.5 * i for i in range(10)],\
                                     senses = ["R"] * 10,\
                                     names = [str(i) for i in range(10)])
        >>> c.linear_constraints.get_num()
        10
    ...
    def get_coefficients(self, *args):
        ...
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = ["x0", "x1"])
        >>> c.linear_constraints.add(names = ["c0", "c1"],\
        ...
        if len(args) == 2:
            ...
            raise CplexError("Wrong number of arguments to linear_constraints.get_coefficients")
    ...
    def get_rows(self, *args):
        ...
        def getrows(begin, end = self.get_num() - 1):
            ...
            m.matval = t[2]
            return [m[i] for i in range(len(m))]
        return apply_freeform_two_args("linear_constraints.get_rows", getrows, self.get_indices, args)
    ...
    def get_names(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.linear_constraints.add(names = ["c" + str(i) for i in range(10)])
        >>> c.linear_constraints.get_num()
        10
...
class IndicatorConstraintInterface(IndexedInterface):
    ...
    def add(self, lin_expr = SparsePair(), sense = "E", rhs = 0.0, indvar = 0, complemented = 0, name = ""):
        ...
        >>> c.indicator_constraints.add(indvar = "x1",\
                                        complemented = 0,\
                                        rhs = 1.0,\
                                        sense = "G",\
        ...
        """
        if isinstance(lin_expr, SparsePair):
            ind = lin_expr.ind
            val = lin_expr.val
        else:
            ind = lin_expr[0]
            val = lin_expr[1]
        if len(ind) != len(val):
            raise CplexError("Inconsistent arguments to indicator_constraints.add")
        CPX_PROC.addindconstr(self._env._e, self._cplex._lp, self._cplex.variables._conv(indvar), complemented,
                              rhs, sense, map(self._cplex.variables._conv, ind), val, name, self._env.parameters.read.apiencoding.get())
    ...
    def get_indicator_variables(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(11)], types = "B" * 11)
        >>> [c.indicator_constraints.add(name = str(i), indvar = i,\
        ...
        """
        def getindvar(a):
            return CPX_PROC.getindconstr_constant(self._env._e, self._cplex._lp, a)[0]
        return apply_freeform_one_arg("indicator_constraints.get_indicator_variables", getindvar, self.get_indices,
                                      CPX_PROC.getnumindconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_complemented(self, *args):
        ...
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(11)], types = "B" * 11)
        >>> [c.indicator_constraints.add(name = str(i), indvar = 10,\
                                         complemented = i % 2)\
                                         for i in range(10)]
        [None, None, None, None, None, None, None, None, None, None]
        >>> c.indicator_constraints.get_num()
        ...
        """
        def getcomp(a):
            return CPX_PROC.getindconstr_constant(self._env._e, self._cplex._lp, a)[1]
        return apply_freeform_one_arg("indicator_constraints.get_complemented", getcomp, self.get_indices,
                                      CPX_PROC.getnumindconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_num_nonzeros(self, *args):
        ...
        """
        def getnnz(a):
            return CPX_PROC.getindconstr_constant(self._env._e, self._cplex._lp, a)[2]
        return apply_freeform_one_arg("indicator_constraints.get_num_nonzeros", getnnz, self.get_indices,
                                      CPX_PROC.getnumindconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_rhs(self, *args):
        ...
        """
        def getrhs(a):
            return CPX_PROC.getindconstr_constant(self._env._e, self._cplex._lp, a)[3]
        return apply_freeform_one_arg("indicator_constraints.get_rhs", getrhs, self.get_indices,
                                      CPX_PROC.getnumindconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_senses(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.indicator_constraints.add(name = "0", sense = "E")
        >>> c.indicator_constraints.add(name = "1", sense = "G")
        >>> c.indicator_constraints.add(name = "2", sense = "L")
        >>> c.indicator_constraints.add(name = "3", sense = "E")
        >>> c.indicator_constraints.get_num()
        4
        ...
        """
        def getsense(a):
            return CPX_PROC.getindconstr_constant(self._env._e, self._cplex._lp, a)[4]
        return apply_freeform_one_arg("indicator_constraints.get_senses", getsense, self.get_indices,
                                      CPX_PROC.getnumindconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_linear_components(self, *args):
        ...
        def getlin(a):
            ...
            return SparsePair(ret[4], ret[5])
        return apply_freeform_one_arg("indicator_constraints.get_linear_components", getlin, self.get_indices,
                                      CPX_PROC.getnumindconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_names(self, *args):
        ...
        """
        def getname(a):
            return CPX_PROC.getindconstrname(self._env._e, self._cplex._lp, a, self._env.parameters.read.apiencoding.get())
        return apply_freeform_one_arg("indicator_constraints.get_names", getname, self.get_indices,
                                      CPX_PROC.getnumindconstrs(self._env._e, self._cplex._lp), args)
...
class QuadraticConstraintInterface(IndexedInterface):
    ...
    def get_num(self):
        ...
        >>> [c.quadratic_constraints.add(name = str(i), lin_expr = l, quad_expr = q) for i in range(10)]
        [None, None, None, None, None, None, None, None, None, None]
    ...
    def add(self, lin_expr = SparsePair([0], [0.0]), quad_expr = SparseTriple([0], [0], [0.0]), sense = "L", rhs = 0.0, name = ""):
        ...
        if isinstance(lin_expr, SparsePair):
            ...
            val = lin_expr[1]
        if len(ind) != len(val):
            raise CplexError("Inconsistent arguments to quadratic_constraints.add")
        if len(val) == 1 and val[0] == 0.0:
            ind = []
            val = []
        if isinstance(quad_expr, SparseTriple):
            ind1 = quad_expr.ind1
            ind2 = quad_expr.ind2
            qval = quad_expr.val
        else:
            ...
            ind2 = quad_expr[1]
            qval = quad_expr[2]
        if len(ind1) != len(qval) or len(ind1) != len(ind2):
            raise CplexError("Inconsistent arguments to quadratic_constraints.add")
        CPX_PROC.addqconstr(self._env._e, self._cplex._lp, rhs, sense, map(self._cplex.variables._conv, ind), val,
                            map(self._cplex.variables._conv, ind1), map(self._cplex.variables._conv, ind2), qval, name, self._env.parameters.read.apiencoding.get())
    ...
    def get_rhs(self, *args):
        ...
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(10)])
        >>> [c.quadratic_constraints.add(rhs = 1.5 * i, name = str(i)) for i in range(10)]
        [None, None, None, None, None, None, None, None, None, None]
        >>> c.quadratic_constraints.get_num()
        10
        ...
        """
        def getrhs(a):
            return CPX_PROC.getqconstr_rhs_sense(self._env._e, self._cplex._lp, a)[0]
        return apply_freeform_one_arg("quadratic_constraints.get_rhs", getrhs, self.get_indices,
                                      CPX_PROC.getnumqconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_senses(self, *args):
        ...
        >>> c.quadratic_constraints.add(name = "0", sense = "G")
        >>> c.quadratic_constraints.add(name = "1", sense = "G")
        >>> c.quadratic_constraints.add(name = "2", sense = "L")
        >>> c.quadratic_constraints.add(name = "3", sense = "L")
        >>> c.quadratic_constraints.get_num()
        4
        ...
        """
        def getsense(a):
            return CPX_PROC.getqconstr_rhs_sense(self._env._e, self._cplex._lp, a)[1]
        return apply_freeform_one_arg("quadratic_constraints.get_senses", getsense, self.get_indices,
                                      CPX_PROC.getnumqconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_linear_num_nonzeros(self, *args):
        ...
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(11)], types = "B" * 11)
        >>> [c.quadratic_constraints.add(name = str(i),\
                                         lin_expr = [range(i), [1.0 * (j+1.0) for j in range(i)]])\
                                         for i in range(10)]
        [None, None, None, None, None, None, None, None, None, None]
        >>> c.quadratic_constraints.get_num()
        ...
        """
        def getlinnz(a):
            return CPX_PROC.getqconstr_linnz(self._env._e, self._cplex._lp, a)
        return apply_freeform_one_arg("quadratic_constraints.get_linear_num_nonzeros", getlinnz, self.get_indices,
                                      CPX_PROC.getnumqconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_linear_components(self, *args):
        ...
        """
        def getlin(a):
            return SparsePair(*CPX_PROC.getqconstr_lin(self._env._e, self._cplex._lp, a))
        return apply_freeform_one_arg("quadratic_constraints.get_linear_components", getlin, self.get_indices,
                                      CPX_PROC.getnumqconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_quad_num_nonzeros(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(11)])
        >>> [c.quadratic_constraints.add(name = str(i),\
        ...
        """
        def getquadnz(a):
            return CPX_PROC.getqconstr_quadnz(self._env._e, self._cplex._lp, a)
        return apply_freeform_one_arg("quadratic_constraints.get_quadear_nonzeros", getquadnz, self.get_indices,
                                      CPX_PROC.getnumqconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_quadratic_components(self, *args):
        ...
        """
        def getquad(a):
            return SparseTriple(*CPX_PROC.getqconstr_quad(self._env._e, self._cplex._lp, a))
        return apply_freeform_one_arg("quadratic_constraints.get_quad", getquad, self.get_indices,
                                      CPX_PROC.getnumqconstrs(self._env._e, self._cplex._lp), args)
    ...
    def get_names(self, *args):
        ...
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(11)])
        >>> [c.quadratic_constraints.add(name = "q" + str(i),\
                                         quad_expr = [range(i), range(i), [1.0 * (j+1.0) for j in range(i)]])\
                                         for i in range(1, 11)]
        [None, None, None, None, None, None, None, None, None, None]
        >>> c.quadratic_constraints.get_num()
        ...
        """
        def getname(a):
            return CPX_PROC.getqconstrname(self._env._e, self._cplex._lp, a, self._env.parameters.read.apiencoding.get())
        return apply_freeform_one_arg("quadratic_constraints.get_names", getname, self.get_indices,
                                      CPX_PROC.getnumqconstrs(self._env._e, self._cplex._lp), args)
...
class SOSType:
    """Constants defining the type of special ordered sets.
       ...
       For a definition of SOS type 1 and 2, see those topics
...
class SOSInterface(IndexedInterface):
    ...
    def add(self, type = "1", SOS = SparsePair([0], [0.0]), name = ""):
        ...
        >>> c.SOS.add(type = "1", name = "type_one",\
                      SOS = cplex.SparsePair(ind = ["2", "3"], val = [25.0, 18.0]))
        >>> c.SOS.add(type = "2", name = "type_two",\
                      SOS = cplex.SparsePair(ind = ["2", "4", "7", "3"], val = [1.0, 3.0, 25.0, 18.0]))

        """
        if isinstance(SOS, SparsePair):
            indices = SOS.ind
            weights = SOS.val
        else:
            indices = SOS[0]
        ...
        if len(indices) != len(weights):
            raise CplexError("Inconsistent arguments to SOS.add")
        CPX_PROC.addsos(self._env._e, self._cplex._lp, type, [0],
                        map(self._cplex.variables._conv, indices), weights, [name], self._env.parameters.read.apiencoding.get())
    ...
    def get_sets(self, *args):
        ...
        return apply_freeform_two_args("SOS.get_sets", getsos, self.get_indices, args)
    ...
    def get_types(self, *args):
        ...
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(11)], types = "B" * 11)
        >>> [c.SOS.add(name = str(i), type = str(i % 2 + 1)) for i in range(10)]
        [None, None, None, None, None, None, None, None, None, None]
        >>> c.SOS.get_num()
        10
        >>> c.SOS.get_types(8)
        ...
        t = [i for i in "".join(apply_freeform_two_args("SOS.get_types", gettype, self.get_indices, args))]
        if len(t) == 1:
            return t[0]
        else:
            return t
    ...
    def get_num_members(self, *args):
        ...
        return apply_freeform_one_arg("SOS.get_num_members", getsize, self.get_indices,
                                      self.get_num(), args)
    ...
    def get_names(self, *args):
        ...
        """
        def getname(a, b = self.get_num() - 1):
            return CPX_PROC.getsosname(self._env._e, self._cplex._lp, a, b, self._env.parameters.read.apiencoding.get())
...
class MIPStartsInterface(IndexedInterface):
    ...
    def get_num(self):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(11)], types = "I" * 11)
        >>> c.MIP_starts.add([(cplex.SparsePair(ind = [i], val = [0.0]),\
    ...
    def add(self, *args):
        ...
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(11)], types = "I" * 11)
        >>> c.MIP_starts.add(cplex.SparsePair(ind = [0], val = [0.0]), c.MIP_starts.effort_level.repair, "first")
        >>> c.MIP_starts.add(cplex.SparsePair(ind = [1], val = [0.0]), c.MIP_starts.effort_level.solve_MIP)
        >>> c.MIP_starts.add([([[2, 4], [0.0, 1.0]], c.MIP_starts.effort_level.auto, "third"),\
                              ([[3, 4], [1.0, 3.0]], c.MIP_starts.effort_level.check_feasibility)])
        >>> c.MIP_starts.get_num()
        4
        ...
        if len(args) == 1:
            ...
            if len(args) == 2:
                ...
                raise CplexError("Wrong number of arguments to MIP_starts.add")
            if isinstance(args[0], SparsePair):
                ind = args[0].ind
                val = args[0].val
            else:
                ind = args[0][0]
                val = args[0][1]
            if len(ind) != len(val):
                raise CplexError("Inconsistent arguments to MIP_starts.add")
            CPX_PROC.addmipstarts(self._env._e, self._cplex._lp, [0],
                                  map(self._cplex.variables._conv, ind), val, [args[1]], [name], self._env.parameters.read.apiencoding.get())
    ...
    def change(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = ["x" + str(i) for i in range(11)], types = "I" * 11)
        >>> c.MIP_starts.add([(cplex.SparsePair(ind = [i], val = [0.0]),\
        ...
        if len(args) == 3:
            if isinstance(args[1], SparsePair):
                ...
                val = args[1][1]
            if len(ind) != len(val):
                raise CplexError("Inconsistent arguments to MIP_starts.change")
            CPX_PROC.chgmipstarts(self._env._e, self._cplex._lp, [self._conv(args[0])], [0],
                                  map(self._cplex.variables._conv, ind), val, [args[2]])
        elif len(args) == 1:
            for arg in args[0]:
    ...
    def delete(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = ['x','y'], types = ["II"])
        >>> c.MIP_starts.add([(cplex.SparsePair(ind = ['x'],val = [1.0]), c.MIP_starts.effort_level.auto, str(i)) for i in range(10)])
        >>> c.MIP_starts.get_num()
        10
    ...
    def get_starts(self, *args):
        ...
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(11)], types = "B" * 11)
        >>> c.MIP_starts.add([(cplex.SparsePair(ind = [i], val = [1.0 * i]),\
                               c.MIP_starts.effort_level.auto, str(i)) for i in range(10)])
        >>> c.MIP_starts.get_num()
        10
        ...
        def getmst(a, b = self.get_num() - 1):
            ...
            m.matind = ret[1]
            m.matval = ret[2]
            return [(m[i], ret[3][i]) for i in range(len(m))]
        return apply_freeform_two_args("MIP_starts.get_starts", getmst, self.get_indices, args)
    ...
    def get_num_entries(self, *args):
        ...
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(11)], types = "B" * 11)
        >>> c.MIP_starts.add([(cplex.SparsePair(ind = range(i), val = [0.0] * i),\
                               c.MIP_starts.effort_level.auto, str(i - 1)) for i in range(1, 11)])
        >>> c.MIP_starts.get_num()
        10
    ...
    def get_names(self, *args):
        ...
        >>> c.variables.add(names = [str(i) for i in range(11)], types = "B" * 11)
        >>> c.MIP_starts.add([(cplex.SparsePair(ind = range(i), val = [0.0] * i),\
                               c.MIP_starts.effort_level.auto, "mst" + str(i - 1)) for i in range(1, 11)])
        >>> c.MIP_starts.get_num()
        10
...
class ObjectiveInterface(BaseInterface):
    ...
    def set_linear(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = [str(i) for i in range(4)])
    ...
    def set_quadratic(self, *args):
        ...
        if isinstance(args[0], _HBMatrix):
            CPX_PROC.copyquad(self._env._e, self._cplex._lp, args[0].matbeg,
                              map(self._cplex.variables._conv, args[0].matind),
                              args[0].matval)
    ...
    def set_quadratic_coefficients(self, *args):
        ...
        """
        if isinstance(self._cplex.variables._conv(args[0]), type(0)):
    ...
    def get_linear(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(obj = [1.5 * i for i in range(10)],\
                            names = [str(i) for i in range(10)])
        >>> c.variables.get_num()
        10
    ...
    def get_quadratic_coefficients(self, *args):
        ...
        if len(args) == 2:
            ...
            raise CplexError("Wrong number of arguments to objective.get_quadratic_coefficients")
...
class ProgressInterface(AdvancedInterface):
    ...
    def get_num_barrier_iterations(self):
        ...
        >>> c.read("qcp.lp")
        >>> c.solve()
        >>> c.solution.progress.get_num_barrier_iterations()
        8
    ...
    def get_num_sifting_iterations(self):
        ...
        >>> c.solve()
        >>> c.solution.progress.get_num_sifting_iterations()
        3
    ...
    def get_num_phase_one_iterations(self):
        ...
        >>> c.read("lpex.mps")
        >>> c.solve()
        >>> c.solution.progress.get_num_phase_one_iterations()
        4
    ...
    def get_num_sifting_phase_one_iterations(self):
        ...
        >>> c.parameters.lpmethod.set(c.parameters.lpmethod.values.sifting)
        >>> c.solve()
        >>> c.solution.progress.get_num_sifting_phase_one_iterations()
        0
    ...
    def get_num_nodes_remaining(self):
        ...
        >>> c.read("ind.lp")
        >>> c.solve()
        >>> c.solution.progress.get_num_nodes_remaining()
        0
    ...
    def get_num_primal_push(self):
        ...
        >>> c.solve()
        >>> c.solution.progress.get_num_primal_push()
        0
    ...
    def get_num_primal_exchange(self):
        ...
        >>> c.parameters.lpmethod.set(c.parameters.lpmethod.values.barrier)
        >>> c.solve()
        >>> c.solution.progress.get_num_primal_exchange()
        1
    ...
    def get_num_dual_push(self):
        ...
        >>> c.parameters.lpmethod.set(c.parameters.lpmethod.values.barrier)
        >>> c.solve()
        >>> c.solution.progress.get_num_dual_push()
        3
    ...
    def get_num_dual_exchange(self):
        ...
        >>> c.parameters.lpmethod.set(c.parameters.lpmethod.values.barrier)
        >>> c.solve()
        >>> c.solution.progress.get_num_dual_exchange()
        0
...
class InfeasibilityInterface(AdvancedInterface):
    ...
    def linear_constraints(self, x, *args):
        ...
        >>> c.read("lpex.mps")
        >>> c.solve()
        >>> c.solution.infeasibility.linear_constraints(c.solution.get_values(), "c10") 
        -0.0
        >>> c.solution.infeasibility.linear_constraints(c.solution.get_values(), 7)
        -0.0
        >>> c.solution.infeasibility.linear_constraints(c.solution.get_values(), ["c13", 4])
        [-0.0, -0.0]
        >>> lconstraint = c.solution.infeasibility.linear_constraints(c.solution.get_values())
        >>> lconstraint[5]
        -0.0
    ...
    def quadratic_constraints(self, x, *args):
        ...
        >>> c.read("miqcp.lp")
        >>> c.solve()
        >>> c.solution.infeasibility.quadratic_constraints(c.solution.get_values(), 2)
        -0.0
        >>> c.solution.infeasibility.quadratic_constraints(c.solution.get_values(), "QC3")
        -0.0
        >>> c.solution.infeasibility.quadratic_constraints(c.solution.get_values(), [1, "QC1"])
        [0.0, -0.0]
        >>> c.solution.infeasibility.quadratic_constraints(c.solution.get_values())
        [-0.0, 0.0, -0.0, -0.0]
...
class CutType:
    ...
    Can be iterated over, as in
    
    for cuttype in solution.MIP.cut_type:
    print solution.MIP.get_num_cuts(cuttype)
    ...
    local_cover           = _constants.CPX_CUT_LOCALCOVER
    tighten               = _constants.CPX_CUT_TIGHTEN
    objective_disjunctive = _constants.CPX_CUT_OBJDISJ
    ...
    def __iter__(self):
        return range(self.__num_types).__iter__()
...
class MIPSolutionInterface(AdvancedInterface):
    ...
    def get_best_objective(self):
        """Returns the currently best known bound 
           on the optimal solution value of a MIP problem. 
...
class BasisInterface(AdvancedInterface):
    ...
    def get_basic_col_index(self, col):
        """Returns the position of a basic structural variable 
        in the basis header.
    ...
    def get_primal_norms(self):
        ...
        >>> pnorm[1]
        [1.7226562499999998, 1.69140625, 2.0, 1.0624999999999996]
...
class SensitivityInterface(AdvancedInterface):
    ...
    def lower_bounds(self, *args):
        ...
        def sa(a, b = self._cplex.variables.get_num() - 1):
            return zip(*CPX_PROC.boundsa_lower(self._env._e, self._cplex._lp, a, b))
    ...
    def upper_bounds(self, *args):
        ...
        (42.5, 1e+20)
        >>> c.solution.sensitivity.upper_bounds(["x3", 0])
        [(42.5, 1e+20), (36.428571428571431, 155.0)]
        >>> c.solution.sensitivity.upper_bounds()
        [(36.428571428571431, 155.0), (17.5, 1e+20), (42.5, 1e+20), (0.625, 1e+20)]

        """
        def sa(a, b = self._cplex.variables.get_num() - 1):
            return zip(*CPX_PROC.boundsa_upper(self._env._e, self._cplex._lp, a, b))
    ...
    def bounds(self, *args):
        ...
        """
        def sa(a, b = self._cplex.variables.get_num() - 1):
            return zip(*CPX_PROC.boundsa(self._env._e, self._cplex._lp, a, b))
    ...
    def objective(self, *args):
        ...
        """
        def sa(a, b = self._cplex.variables.get_num() - 1):
            return zip(*CPX_PROC.objsa(self._env._e, self._cplex._lp, a, b))
    ...
    def rhs(self, *args):
        ...
        >>> c.read("example.mps")
        >>> c.solve()
        >>> c.solution.sensitivity.rhs(1)
        (20.0, 46.666666666666671)
        >>> c.solution.sensitivity.rhs("c3")
        (-1e+20, 112.5)
        >>> c.solution.sensitivity.rhs(["c3", 1])
        [(-1e+20, 112.5), (20.0, 46.666666666666671)]
        >>> c.solution.sensitivity.rhs()
        [(16.666666666666668, 70.0), (20.0, 46.666666666666671), (-1e+20, 112.5), (-1e+20, 42.5)]

        """
        def sa(a, b = self._cplex.linear_constraints.get_num() - 1):
            return zip(*CPX_PROC.rhssa(self._env._e, self._cplex._lp, a, b))
...
class SolnPoolFilterInterface(AdvancedInterface):
    ...
    def add_diversity_filter(self, lb, ub, expression, weights, name):
        ...
        >>> c.read("ind.lp")
        >>> c.solve()
        >>> c.solution.pool.filter.add_diversity_filter(300, 600, [['x1','x2'], [1,1]], [2,1], "")

        """
        if not isinstance(expression, SparsePair):
            ind = expression[0]
            val = expression[1]
        else:
            ind = expression.ind
            val = expression.val
        if len(ind) != len(val):
            raise CplexError("Inconsistent arguments to solution.pool.filter.add_diversity_filter")
        if len(ind) != len(val) or len(ind) != len(weights):
            raise CplexError("Inconsistent input data to solution.pool.filter.add_range")
        CPX_PROC.addsolnpooldivfilter(self._env._e, self._cplex._lp, lb, ub,
                                      map(self._cplex.variables._conv, ind), weights, val, name, self._env.parameters.read.apiencoding.get())
    ...
    def add_range_filter(self, lb, ub, expression, name):
        ...
        >>> c.read("ind.lp")
        >>> c.solve()
        >>> c.solution.pool.filter.add_range_filter(300, 600, [['x1','x2'], [1,1]], "")
        ...
        if not isinstance(expression, SparsePair):
            ...
            val = expression.val
        if len(ind) != len(val):
            raise CplexError("Inconsistent arguments to solution.pool.filter.add_range_filter")
        if len(ind) != len(val):
            raise CplexError("Inconsistent input data to solution.pool.filter.add_range")
        CPX_PROC.addsolnpoolrngfilter(self._env._e, self._cplex._lp, lb, ub,
                                      map(self._cplex.variables._conv, ind), val, name, self._env.parameters.read.apiencoding.get())
    ...
    def get_diversity_filters(self, *args):
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = ['x','y'], types = ["BB"])
        >>> f = cplex.SparsePair(ind = ['x'],val = [1.0])
        >>> [c.solution.pool.filter.add_diversity_filter(0, 1, f, [1], str(i)) for i in range(2)]
        [None, None]
    ...
    def get_range_filters(self, *args):
        ...
        >>> c.variables.add(names = ['x','y'], types = ["II"])
        >>> f = cplex.SparsePair(ind = ['x'],val = [1.0])
        >>> [c.solution.pool.filter.add_range_filter(0.0, 1.0, f, str(i)) for i in range(2)]
        [None, None]
    ...
    def delete(self, *args):
        ...
        >>> c.variables.add(names = ['x','y'], types = ["II"])
        >>> f = cplex.SparsePair(ind = ['x'],val = [1.0])
        >>> [c.solution.pool.filter.add_range_filter(0.0, 1.0, f, str(i)) for i in range(10)]
        [None, None, None, None, None, None, None, None, None, None]
    ...
    def get_indices(self, name):
        """Returns the indices of a set of filters, given their names.

        If name is a string, get_indices returns the index of the
        pool filter with that name.  If no such filter exists, an
        exception is raised.

        If name is a sequence of strings, get_indices returns a list
        of the filter indices corresponding to the strings in name.
        ...
        >>> [c.solution.pool.filter.add_range_filter(0.0, 1.0, f, str(i)) for i in range(10)]
        [None, None, None, None, None, None, None, None, None, None]
        >>> c.solution.pool.filter.get_indices("6")
        6
        >>> c.solution.pool.filter.get_indices(["5", "3"])
        [5, 3]

        """
        if isinstance(name, type("")):
            return CPX_PROC.getsolnpoolfilterindex(self._env._e, self._cplex._lp, name, self._env.parameters.read.apiencoding.get())
        elif isinstance(name, type(u"")):
            return CPX_PROC.getsolnpoolfilterindex(self._env._e, self._cplex._lp, CPX_PROC.cpx_decode(name, CPX_PROC.cpx_default_encoding), CPX_PROC.cpx_default_encoding)
        else:
            return map(lambda a:  CPX_PROC.getsolnpoolfilterindex(self._env._e, self._cplex._lp, a, self._env.parameters.read.apiencoding.get()), name)
    ...
    def get_names(self, *args):
        ...
        """
        def getname(a):
            return CPX_PROC.getsolnpoolfiltername(self._env._e, self._cplex._lp, a, self._env.parameters.read.apiencoding.get())
        return apply_freeform_one_arg("solution.pool.filters.get_names", getname,
                                      self.get_indices, self.get_num(), args)
    ...
    def __str__(self):
        ...
        if self.quality_type == "feasopt":
            ...
            if hasattr(self, "max_kappa"):
                ...
                if self.kappa_attention > 1.0e-6:
                    ret = ret + "Attention level:                  %f\n" % self.kappa_attention
                    ret = ret + "CPLEX encountered numerical difficulties while solving this model.\n"
        return ret
...
class SolnPoolInterface(AdvancedInterface):
    ...
    def get_values(self, soln, *args):
        ...
        236.99999999999991
        >>> c.solution.pool.get_values(1, "x2")
        1.0
        >>> c.solution.pool.get_values(1, ["x2", 2])
        [1.0, 236.99999999999991]
    ...
    def get_integer_quality(self, soln, which):
        ...
        >>> c.read("ind.lp")
        >>> c.solve()
        >>> maxindinf = c.solution.pool.get_integer_quality(1, \
                                  c.solution.pool.quality_metric.max_indicator_slack_infeasibility)
        >>> maxindinf == 2 or maxindinf == 6
        True

        """
        if not isinstance(soln, type(0)):
            soln = self.get_indices(soln)
        if isinstance(which, type(0)):
            return CPX_PROC.getsolnpoolintquality(self._env._e, self._cplex._lp, soln, which)
        else:
            return map(lambda a: CPX_PROC.getsolnpoolintquality(self._env._e, self._cplex._lp, soln, a), which)
    ...
    def get_float_quality(self, soln, which):
        ...
        if not isinstance(soln, type(0)):
            soln = self.get_indices(soln)
        if isinstance(which, type(0)):
            return CPX_PROC.getsolnpooldblquality(self._env._e, self._cplex._lp, soln, which)
        else:
            return map(lambda a: CPX_PROC.getsolnpooldblquality(self._env._e, self._cplex._lp, soln, a), which)
    ...
    def get_mean_objective_value(self):
        ...
        >>> c.read("ind.lp")
        >>> c.solve()
        >>> c.solution.pool.get_mean_objective_value()
        519.60000000000014
    ...
    def get_indices(self, name):
        ...
        """
        if isinstance(name, type("")):
            return CPX_PROC.getsolnpoolsolnindex(self._env._e, self._cplex._lp, name, self._env.parameters.read.apiencoding.get())
        else:
            return map(lambda a:  CPX_PROC.getsolnpoolsolnindex(self._env._e, self._cplex._lp, a, self._env.parameters.read.apiencoding.get()), name)
...
class AdvancedSolutionInterface(AdvancedInterface):
    ...
    def get_gradients(self, *args):
        """Returns information useful in post-solution analysis
           after an LP has been solved and a basis is available.
    ...
    def get_quadratic_slacks_from_x(self, x):
        ...
        >>> qslack[0] < 1.e-6 and qslack[0] > -1.e-6
        True
    ...
    def get_linear_reduced_costs_from_pi(self, pi):
        ...
        >>> reducedcost[0] < 1.e-6 and reducedcost[0] > -1.e-6
        True
    ...
    def get_quadratic_reduced_costs_from_pi(self, pi, x):
        ...
        >>> qreducedcost[0] < 1.e-6 and qreducedcost[0] > -1.e-6
        True
    ...
    def get_Driebeek_penalties(self, basic_variables):
        """Returns values known as Driebeek penalties 
           for a sequence of basic variables.
        ...
        return zip(*CPX_PROC.mdleave(self._env._e, self._cplex._lp,
                                     map(self._cplex.variables._conv, basic_variables)))
    
    def get_quadratic_indefinite_certificate(self):
        """Compute a vector x that satifies x'Qx < 0

        Such a vector demonstrates that the matrix Q violates the assumption of 
        positive semi-definiteness, and can be an aid in debugging a user's program if 
        indefiniteness is an unexpected outcome.
    ...
    def dual_farkas(self):
        """Returns Farkas proof of infeasibility for the 
        active LP model after proven infeasibility.

        See CPXdualfarkas in the Callable Library Reference Manual 
        for more detail.
        ...
        >>> out = c.set_results_stream(None)
        >>> out = c.set_log_stream(None)
        >>> c.linear_constraints.add(senses="L", rhs=[-1])
        >>> c.variables.add(lb=[1], ub=[2],columns=[[[0],[1]]])
        >>> c.parameters.lpmethod.set(c.parameters.lpmethod.values.dual)
        >>> c.parameters.preprocessing.presolve.set(c.parameters.preprocessing.presolve.values.off)
        >>> c.solve()
    ...
    def get_diverging_index(self):
        ...
        >>> out = c.set_results_stream(None)
        >>> out = c.set_log_stream(None)
        >>> c.variables.add(obj=[1,1],lb=[1,-cplex.infinity],ub=[2,cplex.infinity])
        >>> c.parameters.lpmethod.set(c.parameters.lpmethod.values.primal)
        >>> c.parameters.preprocessing.presolve.set(c.parameters.preprocessing.presolve.values.off)
        >>> c.solve()
        ...
        >>> out = c.set_results_stream(None)
        >>> out = c.set_log_stream(None)
        >>> c.variables.add(obj=[-1,-1],lb=[1,1],ub=[2,2])
        >>> c.linear_constraints.add(lin_expr = [[[0,1],[-1,-1]]], rhs=[-10], senses="L")
        >>> c.parameters.lpmethod.set(c.parameters.lpmethod.values.dual)
        >>> c.parameters.preprocessing.presolve.set(c.parameters.preprocessing.presolve.values.off)
        >>> c.solve()
        >>> idx = c.solution.advanced.get_diverging_index()
...
class SolutionStatus:
    ...
    MIP_optimal_relaxed_sum        = _constants.CPXMIP_OPTIMAL_RELAXED_QUAD
...
class SolutionInterface(BaseInterface):
    ...
    def get_dual_values(self, *args):
        ...
        >>> c.read("lpex.mps")
        >>> c.solve()
        >>> c.solution.get_dual_values([0, 1])
        [-0.62857142857142856, 0.0]
    ...
    def get_integer_quality(self, which):
        ...
        if isinstance(which, type(0)):
            ...
            return map(lambda a: CPX_PROC.getintquality(self._env._e, self._cplex._lp, a), which)
    ...
    def get_float_quality(self, which):
        ...
        if isinstance(which, type(0)):
            ...
            return map(lambda a: CPX_PROC.getdblquality(self._env._e, self._cplex._lp, a), which)
    ...
    def is_primal_feasible(self):
        ...
        """
        if CPX_PROC.solninfo(self._env._e, self._cplex._lp)[2] == 0:
            return False
        else:
            return True
    ...
    def is_dual_feasible(self):
        ...
        """
        if CPX_PROC.solninfo(self._env._e, self._cplex._lp)[3] == 0:
            return False
        else:
            return True
    ...
    def get_quadratic_activity_levels(self, *args):
        ...
        >>> c.read("qcp.lp")
        >>> c.solve()
        >>> c.solution.get_quadratic_activity_levels()
        [2.0156165232503569]
...
class PresolveInterface(BaseInterface):
    ...
    def crush_formula(self, formula):
        ...
        """
        if isinstance(formula, SparsePair):
            ind = formula.ind
            val = formula.val
        else:
            ind = formula[0]
            val = formula[1]
        if len(ind) != len(val):
            raise CplexError("Inconsistent arguments to presolve.crush_formula")
    ...
    def uncrush_formula(self, pre_formula):
        ...
        """
        if isinstance(pre_formula, SparsePair):
            ind = pre_formula.ind
            val = pre_formula.val
        else:
            ind = pre_formula[0]
            val = pre_formula[1]
        if len(ind) != len(val):
            raise CplexError("Inconsistent arguments to presolve.uncrush_formula")
    ...
    def get_objective_offset(self):
        """Returns the offset between the original and presolved objective functions.
    ...
    def add_rows(self, lin_expr = [], senses = "", rhs = [], names = []):
        ...
        if not isinstance(senses, type("")):
            senses = "".join(senses)
        num_new_rows = validate_arg_lengths(self._env, "presolve.add_rows",
                                             [rhs, senses, names, lin_expr])
        if isinstance(lin_expr, type([])):
            rmat = _HBMatrix(lin_expr)
        CPX_PROC.preaddrows(self._env._e, self._cplex._lp, rhs, senses,
                                    rmat.matbeg, rmat.matind,
                                    rmat.matval, names, self._env.parameters.read.apiencoding.get())
    ...
    def set_objective(self, objective):
        ...
        """
        if isinstance(objective, SparsePair):
            ind = objective.ind
            val = objective.val
        else:
            ind = objective[0]
            val = objective[1]
        if len(ind) != len(val):
            raise CplexError("Inconsistent arguments to presolve.set_objective")
...
class FeasoptInterface(BaseInterface):
    ...
    def __call__(self, *args):
        ...
        """
        if len(args) == 0:
            raise CplexError("Cplex.feasopt() requires at least one argument")
        gpref = []
        gbeg  = []
        ind   = []
        indt  = []
...
class ConflictInterface(BaseInterface):
    ...
    def all_constraints(self):
        """Returns an object instructing the conflict refiner 
           to include all constraints.
    ...
    def linear_constraints(self, *args):
        """Returns an object instructing the conflict refiner
        to include all linear constraints.
    ...
    def quadratic_constraints(self, *args):
        """Returns an object instructing the conflict refiner
        to include all quadratic constraints.
    ...
    def indicator_constraints(self, *args):
        """Returns an object instructing the conflict refiner
        to include all indicator constraints.
    ...
    def SOS_constraints(self, *args):
        """Returns an object instructing the conflict refiner
        to include all SOS constraints.
    ...
    def refine_MIP_start(self, MIP_start, *args):
        ...
        >>> c = cplex.Cplex()
        >>> out = c.set_results_stream(None)
        >>> c.variables.add([1], [0], [0], c.variables.type.binary);
        >>> c.variables.add([2], [0], [0], c.variables.type.binary);
        >>> c.solve();
        >>> c.linear_constraints.add(lin_expr = [[[0, 1], [1.0, 1.0]]], senses = "E", rhs = [2.0]);
        ...
        """
        if len(args) == 0:
            raise CplexError("Cplex.conflict.refine_MIP_start() requires at least two arguments")
        else:
            self.__ext = True
            self.__num_groups = 0
            self.__groups = []
    ...
    def refine(self, *args):
        ...
        """
        if len(args) == 0:
            raise CplexError("Cplex.conflict.refine() requires at least one argument")
        else:
            self.__ext = True
            self.__num_groups = 0
            self.__groups = []
            ...
            for group in args:
                ...
                for con in group[1]:
                    ...
                    indt.append(con[0])
                    ind.append(tran(con[1]))
            CPX_PROC.refineconflictext(self._env._e, self._cplex._lp, gpref, gbeg, ind, indt)
...
class AdvancedCplexInterface(BaseInterface):
    ...
    def pivot_slacks_in(self, which):
        ...
        """
        if isinstance(self._cplex.linear_constraints._conv(which), type(0)):
            CPX_PROC.pivotin(self._env._e, self._cplex._lp,
                             [self._cplex.linear_constraints._conv(which)])
        else:
            CPX_PROC.pivotin(self._env._e, self._cplex._lp,
                             map(self._cplex.linear_constraints._conv, which))
    ...
    def pivot_fixed_variables_out(self, which):
        ...
        """
        if isinstance(self._cplex.variables._conv(which), type(0)):
            CPX_PROC.pivotout(self._env._e, self._cplex._lp,
                             [self._cplex.variables._conv(which)])
        else:
            CPX_PROC.pivotout(self._env._e, self._cplex._lp,
                             map(self._cplex.variables._conv, which))
    ...
    def strong_branching(self, variables, it_limit):
        ...
        def conv(var):
            try:
                ...
                return  -self._cplex.linear_constraints._conv(var) - 1
        return zip(*CPX_PROC.strongbranch(self._env._e, self._cplex._lp,
                                          map(conv, variables), it_limit))
...
class OrderInterface(BaseInterface):
    ...
    def get(self):
        ...
        """
        return zip(*CPX_PROC.getorder(self._env._e, self._cplex._lp))
    ...
    def set(self, order):
        ...
        """
        ord = zip(*order)
        ord[0] = map(self._cplex.variables._conv, ord[0])
...
class InitialInterface(BaseInterface):
    ...
    def set_start(self, col_status, row_status, col_primal, row_primal,
        ...
        >>> import cplex
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = ["v" + str(i) for i in range(5)])
        >>> c.linear_constraints.add(names = ["r" + str(i) for i in range(3)])
        >>> s = c.start.status
    ...
    def set_basis(self, col, row):
        ...
        >>> c = cplex.Cplex()
        >>> c.variables.add(names = ["v" + str(i) for i in range(5)])
        >>> c.linear_constraints.add(names = ["r" + str(i) for i in range(2)])
        >>> s = c.start.status
