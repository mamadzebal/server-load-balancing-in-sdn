...
from cplex._internal import _procedural
...
class SparsePair(object):
    ...
    def __repr__(self):
        ...
        return "".join(["SparsePair(ind = ", repr(self.ind), ", val = ", repr(self.val), ")"])

    def isvalid(self):
        """Tests that ind and val have the same length."""
        if len(self.ind) == len(self.val):
            return True
        else:
            return False
...
class SparseTriple(object):
    ...
    def __repr__(self):
        ...
        return "".join(["SparseTriple(ind1 = ", repr(self.ind1), ", ind2 = ", repr(self.ind2), ", val = ", repr(self.val), ")"])

    def isvalid(self):
        """Tests that ind1, ind2, and val have the same length."""
        if len(self.ind1) == len(self.ind2) and len(self.ind1) == len(self.val):
            return True
        else:
            return False
