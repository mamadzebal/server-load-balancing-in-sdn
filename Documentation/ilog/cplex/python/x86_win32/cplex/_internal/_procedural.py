...
import _pycplex as CR

from _constants import *

import _list_array_utils as LAU
...
def cpx_decode(my_str, enc):
    if enc == cpx_default_encoding and isinstance(my_str, type("")):
        return my_str
    elif isinstance(my_str, type(u"")):
        return my_str.encode(cpx_default_encoding)
    else:
        return unicode(my_str, enc).encode(cpx_default_encoding)

def cpx_encode(my_str, enc):
    if enc == cpx_default_encoding:
        return my_str
    else:
        return unicode(my_str, cpx_default_encoding).encode(enc)

def cpx_decoder(enc):
    return lambda x: cpx_decode(x, enc)
    
def cpx_encoder(enc):
    return lambda x: cpx_encode(x, enc)

def cpx_transcode(name,enc):
    if isinstance(name, unicode):
        return name.encode("utf-8")
    else:
        return unicode(name, enc).encode("utf-8")
                ...
                if isinstance(callback_exception, Exception) and \
                    ...
                    try:
                        if hasattr(callback_exception[1], "args"):
                            callback_exception = callback_exception[0](*callback_exception[1].args)
                        else:
                            callback_exception = callback_exception[0](callback_exception[1])
...
def tuneparam(env, lp, int_param_values, dbl_param_values, str_param_values):
    ...
    status = CR.CPXXtuneparam(env, lp,
                             intcnt, LAU.int_list_to_array(intnum), LAU.int_list_to_array_trunc_int32(intval),
                             dblcnt, LAU.int_list_to_array(dblnum), LAU.double_list_to_array(dblval),
                             strcnt, LAU.int_list_to_array(strnum), strval, tuning_status)
    sigint_swap()
    check_status(env, status)
    return tuning_status.value()

def tuneparamprobset(env, filenames, filetypes, int_param_values, dbl_param_values, str_param_values):
    tuning_status = CR.intPtr()
    intcnt = len(int_param_values)
    dblcnt = len(dbl_param_values)
    strcnt = len(str_param_values)
    ...
    strval = [str_param_values[i][1] for i in range(strcnt)]
    sigint_swap()
    status = CR.CPXXtuneparamprobset(env, len(filenames), filenames, filetypes,
                             intcnt, LAU.int_list_to_array(intnum), LAU.int_list_to_array_trunc_int32(intval),
                             dblcnt, LAU.int_list_to_array(dblnum), LAU.double_list_to_array(dblval),
                             strcnt, LAU.int_list_to_array(strnum), strval, tuning_status)
    sigint_swap()
...
def createprob(env, probname, enc=default_encoding):
    status = CR.intPtr()
    lp     = CR.CPXXcreateprob(env, status, cpx_decode(probname, enc))
    check_status(env, status.value())
    return lp
...
def chgprobname(env, lp, probname, enc=default_encoding):
    status = CR.CPXXchgprobname(env, lp, cpx_decode(probname, enc))
    check_status(env, status)
    return
...
def getprobname(env, lp, enc=default_encoding):
    ...
    if status != CR.CPXERR_NEGATIVE_SURPLUS:
        check_status(env, status)
    if inoutlist == [0]:
        return cpx_encode("", enc)
    status = CR.CPXXgetprobname(env, lp, inoutlist)
    check_status(env, status)
    return cpx_encode(inoutlist[0], enc)
...
def basicpresolve(env, lp):
    ...
    redlb   = CR.doubleArray(numcols)
    redub   = CR.doubleArray(numcols)
    rstat   = CR.intArray(numrows)
    status  = CR.CPXXbasicpresolve(env, lp, redlb, redub, rstat)
    check_status(env, status)
    return (LAU.double_array_to_list(redlb, numcols), LAU.double_array_to_list(redub, numcols),
...
def strongbranch(env, lp, goodlist, itlim):
    ...
    downpen = CR.doubleArray(goodlen)
    uppen   = CR.doubleArray(goodlen)
    sigint_swap()
    status = CR.CPXXstrongbranch(env, lp, goodlen, goodlist, downpen, uppen, itlim)
    sigint_swap()
    check_status(env, status)
    return (LAU.double_array_to_list(downpen, goodlen), LAU.double_array_to_list(uppen, goodlen))
...
def newcols(env, lp, obj, lb, ub, xctype, colname, enc=default_encoding):
    ...
    status = CR.CPXXnewcols(env, lp, ccnt, obj.array,
                            lb.array, ub.array, xctype,
                            map(cpx_decoder(enc), colname))
    check_status(env, status)
    return
...
def chgcolname(env, lp, indices, newnames, enc=default_encoding):
    status = CR.CPXXchgcolname(env, lp, len(indices), LAU.int_list_to_array(indices),
                               map(cpx_decoder(enc), newnames))
    check_status(env, status)
    return

def chgctype(env, lp, indices, xctype):
    status = CR.CPXXchgctype(env, lp, len(indices), LAU.int_list_to_array(indices), xctype)
    check_status(env, status)
    return

def getcolindex(env, lp, colname, enc=default_encoding):
    index = CR.intPtr()
    status = CR.CPXXgetcolindex(env, lp, cpx_decode(colname, enc), index)
    check_status(env, status)
    return index.value()
...
def getcolname(env, lp, begin, end, enc=default_encoding):
    ...
    if status != CR.CPXERR_NEGATIVE_SURPLUS:
        check_status(env, status)
    if inout_list == [0]:
        return map(cpx_encoder(enc), [""] * (end - begin + 1))
    inout_list.extend([begin, end])
    status = CR.CPXXgetcolname(env, lp, inout_list)
    check_status(env, status)
    return map(cpx_encoder(enc), inout_list)

def getctype(env, lp, begin, end):
    numcols = end - begin + 1
    ctype = ""
    for i in range(numcols):
        ctype = "".join([ctype, "?" + str(randrange(0, 1000000000, 1))])
...
def newrows(env, lp, rhs, sense, rngval, rowname, enc=default_encoding):
    ...
    status = CR.CPXXnewrows(env, lp, rcnt, rhs.array,
                            sense, rng.array,
                            map(cpx_decoder(enc), rowname))
    check_status(env, status)
    return

def addrows(env, lp, ccnt, rcnt, nzcnt, rhs, sense, rmat, colname, rowname, enc=default_encoding):
    rhs = LAU.double_C_array(rhs)
    status = CR.CPXXaddrows(env, lp, ccnt, rcnt, nzcnt, rhs.array, 
                            sense, rmat._mat, colname,
                            map(cpx_decoder(enc), rowname))
    check_status(env, status)
    return
...
def chgrowname(env, lp, indices, newnames, enc=default_encoding):
    status = CR.CPXXchgrowname(env, lp, len(indices), LAU.int_list_to_array(indices),
                               map(cpx_decoder(enc), newnames))
    check_status(env, status)
    return
...
def chgrhs(env, lp, indices, values):
    status = CR.CPXXchgrhs(env, lp, len(indices), LAU.int_list_to_array(indices), LAU.double_list_to_array(values))
    check_status(env, status)
    return
...
def chgsense(env, lp, indices, senses):
    status = CR.CPXXchgsense(env, lp, len(indices), LAU.int_list_to_array(indices), senses)
    check_status(env, status)
    return

def getrhs(env, lp, begin, end):
    rhslen = end - begin + 1
    rhs    = CR.doubleArray(rhslen)
...
def getsense(env, lp, begin, end):
    ...
    check_status(env, status)
    return inout_list[0][:end - begin + 1]

def getrngval(env, lp, begin, end):
    rngvallen = end - begin + 1
    rngval    = CR.doubleArray(rngvallen)
...
def getrowindex(env, lp, rowname, enc=default_encoding):
    index = CR.intPtr()
    status = CR.CPXXgetrowindex(env, lp, cpx_decode(rowname, enc), index)
    check_status(env, status)
    return index.value()
...
def addlazyconstraints(env, lp, rhs, sense, rmatbeg, rmatind, rmatval, names, enc=default_encoding):
    status = CR.CPXXaddlazyconstraints(env, lp, len(rmatbeg), len(rmatind),
                                       LAU.double_list_to_array(rhs), sense,
                                       LAU.int_list_to_array(rmatbeg), LAU.int_list_to_array(rmatind),
                                       LAU.double_list_to_array(rmatval),
                                       map(cpx_decoder(enc), names))
    check_status(env, status)
    return

def addusercuts(env, lp, rhs, sense, rmatbeg, rmatind, rmatval, names, enc=default_encoding):
...
def copysos(env, lp, sostype, sosbeg, sosind, soswt, sosname, enc=default_encoding):
    status = CR.CPXXcopysos(env, lp, len(sosbeg), len(sosind), sostype,
                            LAU.int_list_to_array(sosbeg), LAU.int_list_to_array(sosind),
                            LAU.double_list_to_array(soswt),
                            map(cpx_decoder(enc), sosname))
    check_status(env, status)
    return
...
def getsos_info(env, lp, begin, end):
    ...
    for i in range(numsos):
        sostype = "".join([sostype, "?" + str(randrange(0, 1000000000, 1))])
    sosbeg  = CR.intArray(numsos)
    space   = 0
...
def getsos(env, lp, begin, end):
    ...
    space = -surplus.value()
    sosind  = CR.intArray(space)
    soswt   = CR.doubleArray(space)
    status = CR.CPXXgetsos(env, lp, numnz, sostype, sosbeg, sosind, soswt,
                          space, surplus, begin, end)
    check_status(env, status)
    return (sostype[0:numsos], LAU.int_array_to_list(sosbeg, numsos),
            LAU.int_array_to_list(sosind, space), LAU.double_array_to_list(soswt, space))
    
def getsosindex(env, lp, name, enc=default_encoding):
    index  = CR.intPtr()
    status = CR.CPXXgetsosindex(env, lp, cpx_decode(name, enc), index)
    check_status(env, status)
    return index.value()
...
def addindconstr(env, lp, indvar, complemented, rhs, sense, linind, linval, name, enc=default_encoding):
    status = CR.CPXXaddindconstr(env, lp, indvar, complemented, len(linind), rhs, sense,
                                 LAU.int_list_to_array(linind), LAU.double_list_to_array(linval),
                                 cpx_decode(name, enc))
    check_status(env, status)
    return
...
def getindconstr(env, lp, which):
    ...
    nzcnt = CR.intPtr()
    rhs = CR.doublePtr()
    sense = "?" + str(randrange(0, 1000000000, 1))
    space = 0
    linind = LAU.int_list_to_array([])
    linval = LAU.double_list_to_array([])
    surplus = CR.intPtr()
    status = CR.CPXXgetindconstr(env, lp, indvar, complemented, nzcnt, rhs, sense,
                                linind, linval, space, surplus, which)
    if status != CR.CPXERR_NEGATIVE_SURPLUS:
        check_status(env, status)
    if surplus.value() == 0:
        return (indvar.value(), complemented.value(), rhs.value(), sense[0], [], [])
    space = -surplus.value()
    linind = CR.intArray(space)
    linval = CR.doubleArray(space)
    status = CR.CPXXgetindconstr(env, lp, indvar, complemented, nzcnt, rhs, sense,
                                linind, linval, space, surplus, which)
    check_status(env, status)
    return (indvar.value(), complemented.value(), rhs.value(), sense[0],
            LAU.int_array_to_list(linind, space), LAU.double_array_to_list(linval, space))
...
def getindconstr_constant(env, lp, which):
    ...
    if status != CR.CPXERR_NEGATIVE_SURPLUS:
        check_status(env, status)
    return (indvar.value(), complemented.value(), - surplus.value(), rhs.value(), sense[0])

def getindconstrindex(env, lp, name, enc=default_encoding):
    index = CR.intPtr()
    status = CR.CPXXgetindconstrindex(env, lp, cpx_decode(name, enc), index)
    check_status(env, status)
    return index.value()
...
def getindconstrslack(env, lp, begin, end):
    ...
    slacks   = CR.doubleArray(slacklen)
...
def addqconstr(env, lp, rhs, sense, linind, linval, quadrow, quadcol, quadval, name, enc=default_encoding):
    status = CR.CPXXaddqconstr(env, lp, len(linind), len(quadrow), rhs, sense,
                              LAU.int_list_to_array(linind), LAU.double_list_to_array(linval),
                              LAU.int_list_to_array(quadrow), LAU.int_list_to_array(quadcol),
                              LAU.double_list_to_array(quadval), cpx_decode(name, enc))
    check_status(env, status)
    return
...
def getqconstr(env, lp, which):
    ...
    sense      = "?" + str(randrange(0, 1000000000, 1))
    inout_list = [0, 0]
    status = CR.CPXXgetqconstr(env, lp, linnzcnt, quadnzcnt, rhs, sense, inout_list, which)
    if status != CR.CPXERR_NEGATIVE_SURPLUS:
        check_status(env, status)
    if inout_list == [0, 0]:
        return ()
...
def getqconstr_rhs_sense(env, lp, which):
    ...
    rhs       = CR.doublePtr()
    sense     = "?" + str(randrange(0, 1000000000, 1))
    status = CR.CPXXgetqconstr(env, lp, linnzcnt, quadnzcnt, rhs, sense, [0, 0], which)
    if status != CR.CPXERR_NEGATIVE_SURPLUS:
        check_status(env, status)
...
def getqconstrindex(env, lp, name, enc=default_encoding):
    index = CR.intPtr()
    status = CR.CPXXgetqconstrindex(env, lp, cpx_decode(name, enc), index)
    check_status(env, status)
    return index.value()
...
def getquad(env, lp, begin, end):
    ...
    if surplus.value() == 0:
        return []
    space = -surplus.value()
    qmatind = CR.intArray(space)
    qmatval = CR.doubleArray(space)
    status = CR.CPXXgetquad(env, lp, nzcnt, qmatbeg, qmatind, qmatval, space, surplus, begin, end)
    check_status(env, status)
    return (LAU.int_array_to_list(qmatbeg, ncols), LAU.int_array_to_list(qmatind, space),
...
def solution(env, lp):
    ...
    lpstat   = CR.intPtr()
    objval   = CR.doublePtr()
    x        = CR.doubleArray(xlen)
    pi       = CR.doubleArray(pilen)
    slack    = CR.doubleArray(slacklen)
    dj       = CR.doubleArray(djlen)
    ...
    return (lpstat.value(), objval.value(),
            LAU.double_array_to_list(x, xlen), LAU.double_array_to_list(pi, pilen),
            LAU.double_array_to_list(slack, slacklen), LAU.double_array_to_list(dj, djlen))
...
def getrowinfeas(env, lp, x, begin, end):
    ...
    infeasout    = CR.doubleArray(infeasoutlen)
    status = CR.CPXXgetrowinfeas(env, lp, LAU.double_list_to_array(x), infeasout, begin, end)
    check_status(env, status)
...
def getcolinfeas(env, lp, x, begin, end):
    infeasoutlen = end - begin + 1
    infeasout    = CR.doubleArray(infeasoutlen)
    status = CR.CPXXgetcolinfeas(env, lp, LAU.double_list_to_array(x), infeasout, begin, end)
    check_status(env, status)
    return LAU.double_array_to_list(infeasout, infeasoutlen)
...
def getqconstrinfeas(env, lp, x, begin, end):
    infeasoutlen = end - begin + 1
    infeasout    = CR.doubleArray(infeasoutlen)
    status = CR.CPXXgetqconstrinfeas(env, lp, LAU.double_list_to_array(x), infeasout, begin, end)
    check_status(env, status)
    return LAU.double_array_to_list(infeasout, infeasoutlen)
...
def getindconstrinfeas(env, lp, x, begin, end):
    infeasoutlen = end - begin + 1
    infeasout    = CR.doubleArray(infeasoutlen)
    status = CR.CPXXgetindconstrinfeas(env, lp, LAU.double_list_to_array(x), infeasout, begin, end)
    check_status(env, status)
    return LAU.double_array_to_list(infeasout, infeasoutlen)
...
def getsosinfeas(env, lp, x, begin, end):
    infeasoutlen = end - begin + 1
    infeasout    = CR.doubleArray(infeasoutlen)
    status = CR.CPXXgetsosinfeas(env, lp, LAU.double_list_to_array(x), infeasout, begin, end)
    check_status(env, status)
    return LAU.double_array_to_list(infeasout, infeasoutlen)
...
def getbhead(env, lp):
    ...
    head    = CR.intArray(headlen)
    x       = CR.doubleArray(headlen)
    status  = CR.CPXXgetbhead(env, lp, head, x)
    check_status(env, status)
    return (LAU.int_array_to_list(head, headlen), LAU.double_array_to_list(x, headlen))
...
def getpnorms(env, lp):
    numcols = CR.CPXXgetnumcols(env, lp)
    numrows = CR.CPXXgetnumrows(env, lp)
    cnorm   = CR.doubleArray(numcols)
    rnorm   = CR.doubleArray(numrows)
    length  = CR.intPtr()
    status  = CR.CPXXgetpnorms(env, lp, cnorm, rnorm, length)
    check_status(env, status)
    return (LAU.double_array_to_list(cnorm, length.value()), LAU.double_array_to_list(rnorm, numrows))

def getdnorms(env, lp):
    numrows = CR.CPXXgetnumrows(env, lp)
    norm    = CR.doubleArray(numrows)
    head    = CR.intArray(numrows)
    length  = CR.intPtr()
    status  = CR.CPXXgetdnorms(env, lp, norm, head, length)
    check_status(env, status)
    return (LAU.double_array_to_list(norm, length.value()), LAU.int_array_to_list(head, length.value()))
...
def boundsa_lower(env, lp, begin, end):
    listlen = end - begin + 1
    lblower = CR.doubleArray(listlen)
    lbupper = CR.doubleArray(listlen)
    ublower = LAU.double_list_to_array([])
    ubupper = LAU.double_list_to_array([])
    status = CR.CPXXboundsa(env, lp, begin, end, lblower, lbupper, ublower, ubupper)
    check_status(env, status)
    return (LAU.double_array_to_list(lblower, listlen), LAU.double_array_to_list(lbupper, listlen))

def boundsa_upper(env, lp, begin, end):
    listlen = end - begin + 1
    lblower = LAU.double_list_to_array([])
    lbupper = LAU.double_list_to_array([])
    ublower = CR.doubleArray(listlen)
    ubupper = CR.doubleArray(listlen)
    status = CR.CPXXboundsa(env, lp, begin, end, lblower, lbupper, ublower, ubupper)
    check_status(env, status)
    return (LAU.double_array_to_list(ublower, listlen), LAU.double_array_to_list(ubupper, listlen))
...
def boundsa(env, lp, begin, end):
    ...
    check_status(env, status)
    return (LAU.double_array_to_list(lblower, listlen), LAU.double_array_to_list(lbupper, listlen),
            LAU.double_array_to_list(ublower, listlen), LAU.double_array_to_list(ubupper, listlen))

def objsa(env, lp, begin, end):
    listlen = end - begin + 1
    lower   = CR.doubleArray(listlen)
    upper   = CR.doubleArray(listlen)
    status = CR.CPXXobjsa(env, lp, begin, end, lower, upper)
    check_status(env, status)
    return (LAU.double_array_to_list(lower, listlen), LAU.double_array_to_list(upper, listlen))
...
def getconflict(env, lp, n_r, n_c):
    ...
    n_rows = CR.intPtr()
    n_cols = CR.intPtr()
    rowind = CR.intArray(n_r)
    rowstt = CR.intArray(n_r)
    colind = CR.intArray(n_c)
    colstt = CR.intArray(n_c)
...
def getconflictext(env, lp, begin, end):
    ...
    grpstat    = CR.intArray(grpstatlen)
...
def getgrad(env, lp, j):
    numrows = CR.CPXXgetnumrows(env, lp)
    head    = CR.intArray(numrows)
    y       = CR.doubleArray(numrows)
    status = CR.CPXXgetgrad(env, lp, j, head, y)
    check_status(env, status)
    return (LAU.int_array_to_list(head, numrows), LAU.double_array_to_list(y, numrows))

def slackfromx(env, lp, x):
    numrows = CR.CPXXgetnumrows(env, lp)
    slack   = CR.doubleArray(numrows)
...
def qconstrslackfromx(env, lp, x):
    ...
    slack   = CR.doubleArray(numqcon)
    status  = CR.CPXXqconstrslackfromx(env, lp, LAU.double_list_to_array(x), slack)
    check_status(env, status)
    return (LAU.double_array_to_list(slack, numqcon))

def djfrompi(env, lp, pi):
    numcols = CR.CPXXgetnumcols(env, lp)
    dj      = CR.doubleArray(numcols)
...
def mdleave(env, lp, goodlist):
    goodlen   = len(goodlist)
    downratio = CR.doubleArray(goodlen)
    upratio   = CR.doubleArray(goodlen)
    status = CR.CPXXmdleave(env, lp, LAU.int_list_to_array(goodlist), goodlen, downratio, upratio)
    check_status(env, status)
    return (LAU.double_array_to_list(downratio, goodlen), LAU.double_array_to_list(upratio, goodlen))

def qpindefcertificate(env, lp):
    certlen = CR.CPXXgetnumquad(env, lp)
    cert    = CR.doubleArray(certlen)
...
def crushform(env, lp, ind, val):
    ...
    if redlp.value() is None:
        raise CplexError("No presolved problem found")
    numcols = CR.CPXXgetnumcols(env, redlp.value())
    pind    = CR.intArray(numcols)
    pval    = CR.doubleArray(numcols)
    status  = CR.CPXXcrushform(env, lp, len(ind), LAU.int_list_to_array(ind),
                              LAU.double_list_to_array(val), plen, poffset, pind, pval)
    check_status(env, status)
...
def uncrushform(env, lp, pind, pval):
    ...
    ind    = CR.intArray(maxlen)
    val    = CR.doubleArray(maxlen)
    status = CR.CPXXuncrushform(env, lp, len(pind), LAU.int_list_to_array(pind),
                              LAU.double_list_to_array(pval), length, offset, ind, val)
    check_status(env, status)
...
def preaddrows(env, lp, rhs, sense, rmatbeg, rmatind, rmatval, names, enc=default_encoding):
...
def addmipstarts(env, lp, beg, varindices, values, effortlevel, mipstartname, enc=default_encoding):
    status = CR.CPXXaddmipstarts(env, lp, len(beg), len(varindices), LAU.int_list_to_array(beg),
                                 LAU.int_list_to_array(varindices), LAU.double_list_to_array(values),
                                 LAU.int_list_to_array(effortlevel),
                                 map(cpx_decoder(enc), mipstartname))
    check_status(env, status)
    return
...
def getmipstarts_size(env, lp, begin, end):
    ...
    effortlevel = CR.intArray(beglen)
    nzcnt       = CR.intPtr()
    surplus     = CR.intPtr()
...
def getmipstarts_effort(env, lp, begin, end):
    ...
    beg         = CR.intArray(beglen)
    varindices  = CR.intArray(startspace)
    values      = CR.doubleArray(startspace)
    status = CR.CPXXgetmipstarts(env, lp, nzcnt, beg, varindices, values,
                                effortlevel, startspace, surplus, begin, end)
    check_status(env, status)
...
def getmipstartindex(env, lp, mipstartname, enc=default_encoding):
    index = CR.intPtr()
    status = CR.CPXXgetmipstartindex(env, lp, cpx_decode(mipstartname, enc), index)
    check_status(env, status)
    return index.value()
...
def getcallbackpseudocosts(cbstruct, begin, end):
    pclen = end - begin + 1
    uppc  = CR.doubleArray(pclen)
    dnpc  = CR.doubleArray(pclen)
    status = CR.CPXXgetcallbackpseudocosts(cbstruct, uppc, dnpc, begin, end)
    check_status(None, status)
    return (LAU.double_array_to_list(uppc, pclen), LAU.double_array_to_list(dnpc, pclen))

def getcallbacknodeintfeas(cbstruct, begin, end):
    feaslen = end - begin + 1
    feas    = CR.intArray(feaslen)
...
def getcallbacknodeinfo(cbstruct, node, which):
    if which in int_callback_node_info:
        ...
        data = "?" + str(randrange(0, 1000000000, 1))
...
def cutcallbackadd(cbstruct, rhs, sense, ind, val, purgeable):
    status = CR.CPXXcutcallbackadd(cbstruct, len(ind), rhs, sense,
                                  LAU.int_list_to_array(ind),
                                  LAU.double_list_to_array(val), purgeable)
    check_status(None, status)
    return
    
def cutcallbackaddlocal(cbstruct, rhs, sense, ind, val):
    status = CR.CPXXcutcallbackaddlocal(cbstruct, len(ind), rhs, sense,
                                       LAU.int_list_to_array(ind),
                                       LAU.double_list_to_array(val))
    check_status(None, status)
    return

def branchcallbackbranchgeneral(cbstruct, 
                                ind, lu, bd, rhs, sense, matbeg, matind, matval, nodeest, userhandle):
    seqnum = CR.cpxlongPtr()
    status = CR.CPXXbranchcallbackbranchgeneral(cbstruct, len(ind),
                                               LAU.int_list_to_array(ind), lu,
                                               LAU.double_list_to_array(bd), len(matbeg), len(matind),
                                               LAU.double_list_to_array(rhs), sense, 
...
def addsolnpooldivfilter(env, lp, lb, ub, ind, wts, ref, name, enc=default_encoding):
...
def addsolnpoolrngfilter(env, lp, lb, ub, ind, val, name, enc=default_encoding):
...
def getsolnpooldivfilter(env, lp, which):
    ...
    if status != CR.CPXERR_NEGATIVE_SURPLUS:
        check_status(env, status)
    space = -surplus.value()
    ind = CR.intArray(space)
    wts = CR.doubleArray(space)
    ref = CR.doubleArray(space)
    status = CR.CPXXgetsolnpooldivfilter(env, lp, lb, ub, nzcnt, ind, wts, ref,
                                        space, surplus, which)
    check_status(env, status)
    return (lb.value(), ub.value(), LAU.int_array_to_list(ind, space),
            LAU.double_array_to_list(wts, space), LAU.double_array_to_list(ref, space))
...
def getsolnpoolfilterindex(env, lp, colname, enc=default_encoding):
    index = CR.intPtr()
    status = CR.CPXXgetsolnpoolfilterindex(env, lp, cpx_decode(colname, enc), index)
    check_status(env, status)
    return index.value()
...
def getsolnpoolsolnindex(env, lp, colname, enc=default_encoding):
    index = CR.intPtr()
    status = CR.CPXXgetsolnpoolsolnindex(env, lp, cpx_decode(colname, enc), index)
    check_status(env, status)
    return index.value()
...
def getorder(env, lp):
    ...
    space = -surplus.value()
    ind = CR.intArray(space)
    pri = CR.intArray(space)
    dir_ = CR.intArray(space)
    status = CR.CPXXgetorder(env, lp, count, ind, pri, dir_, space, surplus)
    check_status(env, status)
...
def getqualitymetrics(env, lp, soln):
    space  = 26
    data   = CR.doubleArray(space)
    ispace = 10
    idata  = CR.intArray(ispace)
